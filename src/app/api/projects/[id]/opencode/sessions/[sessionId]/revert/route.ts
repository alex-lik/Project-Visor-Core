...
import { revertOpenCodeMessage, unrevertOpenCodeSession, OpenCodeHostConfig } from '@/lib/opencode';
...
export async function POST(
  ...
  try {
    if (action === 'unrevert') {
      const res = await unrevertOpenCodeSession(host as OpenCodeHostConfig, sessionId);
      return NextResponse.json({ success: true, result: res });
    } else {
      if (!body.messageId) {
        return NextResponse.json({ error: 'messageId is required to revert a step' }, { status: 400 });
      }
      const res = await revertOpenCodeMessage(host as OpenCodeHostConfig, sessionId, body.messageId);
      return NextResponse.json({ success: true, result: res });
    }
  } catch (err: any) {
